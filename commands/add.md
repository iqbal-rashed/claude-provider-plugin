---
description: Add a new provider profile with preset or custom configuration (example: /claude-switcher:add kimi)
---
...
- Call MCP tool `claude-switcher.list_presets` to get preset details
- Ask the user for their API key for that provider
- Then call `claude-switcher.add_profile` with the preset and API key

If "$ARGUMENTS" contains "custom":
- Ask the user for: base URL, API key, and model name
- Call `claude-switcher.add_profile` with type "custom" and the provided values
